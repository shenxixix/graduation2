<template>
  <div class="p-4 bg-gray-50 min-h-full flex flex-col">
    ...
    <div class="p-4 bg-white mt-4 flex-1">
      ...
      <div class="mt-4">
        <el-table v-loading="loading" :data="list" border :scrollbar-always-on="true" style="width: 100%">
          ...
          <el-table-column label="车辆类型" align="center" prop="carTypeName" :show-overflow-tooltip="true" min-width="200" />
          ...
          <el-table-column label="用车时间" align="center" prop="pickUpCarTime" :show-overflow-tooltip="true" min-width="180">
            <template #default="scope">
          ...
          <el-table-column label="操作" align="center" width="150" fixed="right">
            <template #default="scope">
              <el-button type="text" @click="handleQuote(scope.row)" class="change">派车</el-button>
            </template>
          </el-table-column>
        </el-table>
    ...
    <Dispatch v-model="quoteVisible" v-if="quoteVisible" :id="keyId" @update="resetQuery"></Dispatch>
    <QuoteDispatch v-model="quoteDispatchVisible" v-if="quoteDispatchVisible" :id="keyId" @update="resetQuery"></QuoteDispatch>
  </div>
</template>

<script setup>
import { dispatchTodoList } from "@/api/deal";
...
const quoteDispatchVisible = ref(false)
const keyId = ref("");
function handleQuote(val) {
  ...
  if(val.type == 1) {
    ...
    quoteDispatchVisible.value = true;
  }
}
...
function handleExport() {
  proxy.download(
    "/zoom-fleet-manager/drivingTodo/exportDispatchTodoList",
    {
      ...queryParams,
      exportType: 1,
    },
