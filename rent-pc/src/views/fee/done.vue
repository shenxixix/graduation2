      ...
      <div class="mt-4">
        <el-table v-loading="loading" :data="list" border :scrollbar-always-on="true" style="width: 100%">
          ...
          <el-table-column label="结算时间" align="center" prop="settlementTime" :show-overflow-tooltip="true"
            min-width="180">
            <template #default="scope">
              <span>{{ formatTime(scope.row.settlementTime) }}</span>
            </template>
          </el-table-column>
...
import { feeSettlementList } from "@/api/fee"
...
function handleExport() {
  proxy.download(
    "/zoom-fleet-manager/settlement/exportFeeSettlementList",
    {
      ...queryParams,
      exportType: 1,
    },
